//
//  TeacherAvailableCoursesTVC.swift
//  Omnivox
//
...
class TeacherAvailableCoursesTVC: UITableViewController {
    ...
    func refreshCourses() {
        ...
        api.getJsonEntities(withURL: "http://localhost:8080/teacher/\(activeTeacherID)/courses") { (response, json) in
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let assignTeacherForm = AssignTeacherForm(teacherID: activeTeacherID, courseID: self.courses[indexPath.row].courseID!)
        api.postJsonEntity(withForm: assignTeacherForm) { (response) in
            if response.statusCode == 200 {
